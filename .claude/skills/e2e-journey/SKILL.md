---
name: e2e-journey
description: Execute E2E user journeys interactively with Playwright MCP. Use when you need to run user journey tests through the actual application with step-by-step verification.
argument-hint: [--journey=UJ-X] [--all] [--headless] [--screenshot] [--production]
user-invocable: true
context: fork
model: opus
---

# E2E User Journey Runner

Execute user journeys step-by-step using Playwright MCP server. This skill provides interactive testing with real-time browser interaction, screenshots, and detailed reporting.
...
| `mcp__plugin_playwright_playwright__browser_snapshot` | Capture page state (preferred over screenshot for analysis) |
...
| `mcp__plugin_playwright_playwright__browser_run_code` | Execute arbitrary Playwright code |
...
- **Status:** ✓ Passed
- **Screenshot:** `test-results/uj1-step2.png`
...
```typescript
// tests/journeys/data/uj-steps.ts
...
export const journeys: Journey[] = [
  {
    ...
    steps: [
      { id: 'UJ-7.1', name: 'Navigate to register', action: 'navigate', target: '/register' },
      { id: 'UJ-7.2', name: 'Enter name', action: 'type', target: 'Name', value: 'Test User' },
      { id: 'UJ-7.3', name: 'Enter email', action: 'type', target: 'Email', value: 'test@example.com' },
      { id: 'UJ-7.4', name: 'Enter password', action: 'type', target: 'Password', value: 'Test1234!' },
      { id: 'UJ-7.5', name: 'Submit form', action: 'click', target: 'Register' },
      { id: 'UJ-7.6', name: 'Verify redirect', action: 'verify', expected: '/dashboard' },
    ]
  }
];
```
...
try {
  ...
  await page.screenshot({ path: `test-results/${step.id}-error.png` });
  const consoleMessages = await mcp__plugin_playwright_playwright__browser_console_messages({ level: 'error' });
...
```
test(PROJ-X): Add E2E journey test results for [journey name]
```